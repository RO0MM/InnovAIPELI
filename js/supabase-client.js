...
async function bootMovies() {
  ...
  if (error) {
    ...
    msg(
      $("#loadStatus"),
      "⚠️ No se pudo cargar el catálogo. El código puede estar vencido o la configuración RLS no está lista.",
      "err"
    );
    return;
  }
...
function filterByKind(kind) {
  ...
  if (kind === "all") {
    ...
    FILTERED = MOVIES.filter(m => isFav(m.id));
  } else {
    FILTERED = MOVIES.filter(m => String(m.kind || "").toLowerCase() === kind);
  }

  const q = searchInput.value.trim().toLowerCase();

  if (q) {
    FILTERED = FILTERED.filter(matchSearch(q));
  }
